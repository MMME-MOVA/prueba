...
async function dashLoadPreop(anio,mes=-1){
  ...
  try{
    ...
    for(const col of cols){
      const snap=await getDcsByProy(col);
      snap.docs.forEach(d=>{
        const data=d.data();
...
async function dashLoadFallas(anio,mes=-1){
  ...
  try{
    const ini=new Date(anio,0,1),fin=new Date(anio+1,0,1);
    const snap=await getDcsByProy('reportes_fallas');
...
async function loadInicioKPIs(){
  ...
  try{
    // ── Fallas del mes ──
    const sfallas=await getDcsByProy('reportes_fallas');
    ...
    const smtto=await getDcsByProy('solicitudes_mtto');
...
async function dashLoadTopKPIs(anio,mes=-1){
  try{
    ...
    try{
      const sdanos=await getDcsByProy('reportes_danos');
